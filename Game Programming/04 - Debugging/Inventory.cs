		...
		void OrganizeInventory() {
			...
			for (int i = 0; i < storage.Length; i++) {
				for (int j = i + 1; j < storage.Length; j++) {
					current = TakeFromInventory(i);
					next = TakeFromInventory(j);

					if (next == null || (current == null && j < i))
						continue;

					if (current == null || Compare(next.name, current.name)) {
						RemoveFromInventory(i);
						PlaceInInventory(i, next);
						PlaceInInventory(j, current);
					}
				}
			}
		}
